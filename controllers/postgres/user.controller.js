...
const userContext = require('../../db/context/postgres/user.context');
...
const importUser = async (req, res) => {
  try {
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      if (existingUser) {
        await userContext.updateUser(existingUser.id, { firstName, lastName, profilePicture, dateOfBirth, gender })
      } else {
...
const exportUser = async (req, res) => {
  try {
    // console.log(Object.keys(UserModel.getAttributes()))
    const xlsxData = [
      [
    ...
    const filePath = path.join(process.cwd(), 'postgresUser.xlsx');
